...
public class GameScreen extends Screen
{
	...
	public final SoldierData[] usableSoldiers;
	...
	public final LuaObject gameObject;
	
	public GameScreen(LDGame game, LuaInterpreter interpreter)
	{
		super(game);
		this.interpreter = interpreter;
		usableSoldiers = interpreter.getExtra("usableSoldiers", SoldierData[].class);
		interpreter.setExtra("gameScreen", this);

		interpreter.importLib(new LuaLibrary<>("Globe", GlobeLibrary.class));

		gameObject = interpreter.require(game.luaFile("game_session.lua"));
		if(!gameObject.isTable())
			throw new IllegalStateException("unexpected result from game_session.lua: " + gameObject);

		unitTab = new UnitTab(this);
		aft = new AffineTransform();
		// Positioning the base globe.
		baseGlobe = new Globe(interpreter, true, Main.WIDTH / 2F, Main.HEIGHT * 4F / 5, 250);
		for(int i = 0; i < 3; i++)
		{
			Soldier s = new FootSoldier(this, baseGlobe);
			s.pos.y = baseGlobe.pos.y;
			s.pos.x = Main.WIDTH / 2F + (i - 1) * 100;
			...
			if(nextGlobe > 0)
			{
				...
				if(nextGlobe == 0)
				{
					...
					if(put)
					{
						...
						comingGlobe = new Globe(interpreter, false, baseGlobe.pos.x + (leftSide ? -250 - rad : 250 + rad), baseGlobe.pos.y, rad);
	...
	public void paintScreen(G2D g2d)
	{
		...
		for (Globe allGlobe : allGlobes)
			allGlobe.paintGlobe(g2d);

		for (Globe allGlobe : allGlobes)
			allGlobe.paintSoldiers(g2d);
		...
		String[] displayResources = player.getDisplayResources();
		int i2 = 0;
		for (String displayResource : displayResources)
			g2d.drawString(displayResource + ": " + fm.format(player.getResource(displayResource)), 5, i2 += 30);
		...
		if(baseGlobe.isDead() || paused)
		{
			...
			if(paused)
			{
				g2d.setColor(Color.BLUE);
				g2d.enable(G2D.G_CENTER);
				g2d.drawString("Paused", x + w / 2F, y + 25);
				g2d.disable(G2D.G_CENTER);
			...
			else if(baseGlobe.isDead())
			{
				g2d.setColor(Color.RED);
				g2d.enable(G2D.G_CENTER);
				g2d.drawString("Game Over", x + w / 2F, y + h / 5F);
				g2d.drawString("Wave: " + difficulty, x + w / 2F, y + h * 2F / 5);
				g2d.drawString("Enemies Killed: " + enemiesKilled, x + w / 2F, y + h * 3F / 5);
				g2d.drawString("(Click anywhere to go to the main menu)", x + w / 2F, y + h * 4F / 5);
				g2d.disable(G2D.G_CENTER);
			}
		}
	}
	...
	{
		shakeAmt = Math.max(amt, 0);
	}
	
	public List<Soldier> genSoldiers(int difficulty, Globe globe)
	{
		List<Soldier> soldiers = new ArrayList<>();
		List<SoldierData> validChoices = new ArrayList<>(Arrays.asList(usableSoldiers));
		...
		if(this.difficulty % 10 == 0)
		{
			for(int i = 0; i < difficulty / 10; i++)
			{
				...
				s.pos.x = globe.pos.x + (Rand.nextFloat(globe.radius * 1.6F) - globe.radius * 0.8F);
				s.pos.y = globe.pos.y;
				soldiers.add(s);
			}
	...
	public void mouse(float x, float y, boolean pressed, int button)
	{
			...
			if(!pressed)
			{
				for(int i = 0; i < pauseButtons.length; i++)
				{
					...
					if(r.contains(x, y))
					{
						...
						{
							interpreter.setExtra("gameScreen", null);
							game.setScreen(new MenuScreen(game, interpreter));
						}
						break;
					}
		...
		else
		{
			if(!pressed && tutorial == 0)
			{
				if(button == MouseEvent.BUTTON1)
				{
					...
					else if(nextGlobe > 0 && x < 200 && y > Main.HEIGHT - 50)
					{
						...
						{
							player.addResource("Wood", (nextGlobe / 20) * baseGlobe.soldiers.size());
							player.addResource("Lives", nextGlobe / 200);
							nextGlobe = 1;
						}
					}
				}
			...
			if(baseGlobe.tower.getHealth() > 0 && tutorial == 0 && aft.createTransformedShape(baseGlobe.tower.bounds).contains(x, y))
			{
				...
				{
					if(player.hasResource("Stone", 50) && baseGlobe.tower.getHealth() < baseGlobe.tower.maxHealth)
					{
						player.takeResource("Stone", 50);
...
//			Shape s = aft.createTransformedShape(baseGlobe.tower.bounds);
			...
			for (Globe allGlobe : allGlobes)
			{
				Shape s = aft.createTransformedShape(allGlobe.tower.bounds);
				if (s.contains(x, y))
				{
					hovX = x;
					hovY = y;
					hovTower = allGlobe.tower;
					return;
				}
			}
			if(handler.isButton(MouseEvent.BUTTON3))
			{
				if(lastPress.x != -1 && lastPress.y != -1)
				{
					...
					move.x = MathUtil.between(-500, move.x, 500);
					move.y = MathUtil.between(-350, move.y, 350);
				}
			}
		}
	}
	...
	public void mouseWheel(int amt)
	{
		...
		if(!paused)
			actualZoom = MathUtil.between(0.1F, actualZoom * (1F - amt / 10F), 10);
	}
		...
		if(!pressed)
		{
			if(!paused)
			{
				...
				if(baseGlobe.left != null || baseGlobe.right != null)
				{
					...
					{
						Soldier s = Rand.<SoldierData>nextFrom(usableSoldiers).createSoldier(this, g);
						s.pos.set(g.pos);
						g.soldiers.add(s);
					}
				}
