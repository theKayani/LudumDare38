...
public class Globe
{
	...
	public Globe(GameScreen scr, boolean friendly, float x, float y, float radius)
	{
		this.scr = scr;
	 ...
	 */
	public void updateGlobe(int ticks)
	{
		tower.updateTower(ticks);

		for(int i = lines.size() - 1; i >= 0; i--)
		{
			if(lines.get(i).updatePiece(ticks))
			{
				lines.remove(i);
			}
		}
	...
	public void paintSoldiers(G2D g2d)
	{
		...
		for(int i = 0; i < soldiers.size(); i++)
		{
			soldiers.get(i).paintSoldier(g2d, false);
		}
	}
	...
	/**
	 * Spawn a random soldier somewhere on this globe.
	 */
	public void spawnRandomSoldier()
	{
		Soldier soldier = ((SoldierData) Rand.nextFrom(Soldier.USABLE_SOLDIERS)).createSoldier(scr, this);
		soldier.pos.set(pos.x + Rand.nextFloat(radius * 1.6F) - radius * 0.8F, pos.y);
